...
// propIntelV2Summarize reduces a v2 response to the compact widget payload.
// Same shape as v1's summary plus three additive per-cell fields (Active
// Sources / Open Agreement / Source Count) so horstapp can migrate to v2
// without breaking older builds.
func propIntelV2Summarize(resp propIntelV2Response) propIntelV2SummaryResponse {
	sum := propIntelV2SummaryResponse{
		Now:          resp.Now,
		QTH:          resp.QTH,
		...
		Grid:         []propIntelV2GridCell{},
	}
	if len(resp.Cells) == 0 {
		sum.Headline = "No propagation paths in the window"
		return sum
	}
	...
	for _, c := range resp.Cells {
		...
		sum.Grid = append(sum.Grid, propIntelV2GridCell{
			Band:      c.Band,
			Region:    c.Region,
			Intensity: intensity,
			Flags:     flags,
			// Additive v2 fields (ignored by old clients).
			ActiveSources: append([]string(nil), c.ActiveSources...),
			OpenAgreement: c.OpenAgreement,
			SourceCount:   len(c.ActiveSources),
		})
	}
	...
	// Headline: a multi-source atypical outranks a single-source one; then
	// rising; then quiet. Same confidence/z tiebreaks as v1.
	var bestAtypical, bestRising *propIntelV2Cell
	for i := range resp.Cells {
		c := &resp.Cells[i]
		if c.Atypical != nil {
			cMulti := c.AtypicalAgreement >= 0.5
			bMulti := bestAtypical != nil && bestAtypical.AtypicalAgreement >= 0.5
			switch {
			case bestAtypical == nil:
				bestAtypical = c
			case cMulti != bMulti:
				// Multi-source agreement always outranks single-source.
				if cMulti {
					bestAtypical = c
				}
			case c.Atypical.Confidence > bestAtypical.Atypical.Confidence:
				bestAtypical = c
			case c.Atypical.Confidence == bestAtypical.Atypical.Confidence && c.Atypical.ZScore > bestAtypical.Atypical.ZScore:
				bestAtypical = c
			}
		}
		if c.Rising {
	...
	switch {
	case bestAtypical != nil && bestAtypical.AtypicalAgreement >= 0.5:
		sum.HeadlineKind = "atypical"
		sum.Headline = fmt.Sprintf("%s multi-source surge to %s (z=%.1f, %d sources)",
			bestAtypical.Band, regionDisplayName(bestAtypical.Region),
			bestAtypical.Atypical.ZScore, len(bestAtypical.ActiveSources))
	case bestAtypical != nil:
		sum.HeadlineKind = "atypical"
...
// propIntelV2SummaryResponse mirrors the v1 summary with additive grid fields.
type propIntelV2SummaryResponse struct {
	Now          int64                 `json:"now"`
	QTH          string                `json:"qth"`
	Minutes      int                   `json:"minutes"`
	FromHere     bool                  `json:"from_here"`
	...
	Grid         []propIntelV2GridCell `json:"grid"`
}

// propIntelV2GridCell is the v1 grid cell plus additive source fields.
type propIntelV2GridCell struct {
	Band      string  `json:"b"`
	Region    string  `json:"r"`
	Intensity float64 `json:"i"`
	Flags     int     `json:"f"`
	// ActiveSources ("sa"), OpenAgreement ("oa"), SourceCount ("src") are the
	// additive v2 fields horstapp's next version reads.
	ActiveSources []string `json:"sa,omitempty"`
	OpenAgreement float64  `json:"oa"`
	SourceCount   int      `json:"src"`
}

// propIntelV2SummaryCache is a separate single-entry cache from v1's — query
// strings collide on shape, not semantics, so the entries are never shared.
var propIntelV2SummaryCache propIntelSummaryCacheEntry

// propIntelV2SummaryHandler serves /api/prop_intel/v2/summary: same engine
// and parameters as /api/prop_intel/v2, reduced to the widget payload and
// cached for 60s.
func propIntelV2SummaryHandler(w http.ResponseWriter, r *http.Request) {
	propIntelAccounting.requests.Add(1)
	p, ok := parsePropIntelParams(r)
	if !ok {
		propIntelAccounting.errors.Add(1)
	...
	key := r.URL.RawQuery
	propIntelV2SummaryCache.mu.Lock()
	if key == propIntelV2SummaryCache.key && time.Since(propIntelV2SummaryCache.at) < propIntelSummaryCacheTTL {
		body := propIntelV2SummaryCache.body
		propIntelV2SummaryCache.mu.Unlock()
		w.Header().Set("Content-Type", "application/json")
	...
	propIntelV2SummaryCache.mu.Lock()
	propIntelV2SummaryCache.key = key
	propIntelV2SummaryCache.body = body
	propIntelV2SummaryCache.at = time.Now()
	propIntelV2SummaryCache.mu.Unlock()
