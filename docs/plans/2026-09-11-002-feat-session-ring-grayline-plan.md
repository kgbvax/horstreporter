...
- AE1. Operator watches a band for 2 hours, opens timeline, rewinds to 90 minutes ago, plays to now.
  - **Covers:** R1, R3, R5, R9.
  - **Given:** SSE connected continuously; **When:** the rewind target and the played window lie within session coverage; **Then:** zero `/api/history` requests fire and the moment matches what a fresh archive bundle would serve.
...
- **KTD-7. Ring coverage contract: strict per-chunk contiguity.** A ring bundle may be synthesized for a chunk iff `ring.minT ≤ chunkT0` and there is no interior gap in `[chunkT0, min(chunkT1, lastReceivedT)]`; the future-side overhang of the chunk containing "now" is harmless because moment windows never extend beyond now. Coverage is tracked in derived spot-time (t = recvMs − ageSeconds), not receive-time intervals — receive-time tracking was rejected as conservative: the connect/reconnect dump's receive interval is a short burst while its content spans up to 60 min earlier in derived t, so it would permanently false-miss the dump-seeded hour and break the U7 fresh-session zero-fetch step. With derived-t coverage, dump back-fill genuinely covers outage gaps shorter than the dump depth; only longer content gaps create real misses. A naive "ring non-empty ⇒ serve" check would render silent holes as dead bands — strictly worse than fetching. Misses fall through to `/api/history` silently (AE4).
...
- **KTD-10. Pause model: dedicated timeline gate, not `state.softPaused`.** `state.softPaused` is owned by the visibility machinery (`syncSoftPauseWithVisibility`, static/app.js:689-729); reusing it would resume live rendering on every refocus mid-replay and clobber the timeline moment. Timeline gets its own gate: while it is active, `scheduleRender` re-emits the current timeline moment (so theme/projection toggles re-render with new styles) and only the live-array render path is suppressed, regardless of caller, the SSE `onmessage` path keeps appending to `liveSpots` and the ring, and `syncSoftPauseWithVisibility` becomes timeline-aware (early-return during timeline). `resumeFromSoftPause` stays visibility-only.
- **KTD-11. Exit = rebuild from the ring tail.** On exit, `state.liveSpots` is rebuilt from the ring's last live-window (deduped, t-sorted) instead of `startLiveStream(false)` — no server dump (R6). This also bypasses the `resumeFromSoftPause` bulk-aging bug (static/app.js:707-716 adds the whole pause duration to every spot's stepped `ageSeconds`, which would prune everything received during a multi-hour replay). Filter changes made during timeline are reconciled with the stream filter on exit (widening queued during replay, reconciled via the existing `restartStreamIfSubscribed` semantics). A qth or surroundings change mid-timeline invalidates the whole ring (KTD-13 keys the ring on both) and exits to one full re-stream (accepted dump; the surroundings handler, app.js:1633-1639, otherwise dispatches a mid-replay stream restart). Fatal SSE drop mid-timeline: one silent stream restart attempt (the ring absorbs the reconnect dump); if it fails again, surface ring staleness in the app's existing stream-status/error surface, with copy distinguishing "ring coverage ended" from a plain connection error, rather than serving stale-covered moments silently.
...
| Surroundings toggle | full re-stream | exits timeline, clears ring, one full re-stream (same treatment; KTD-13 keys the ring on surroundings) |
...
U1 → U2 → U3 → U4 → U5 → U6 → U7 (U4's mid-timeline handler wiring needs U3's bundle-invalidation path; U4's enter/exit/gate work is independent of U3 and may proceed in parallel if the handler wiring is deferred until after U3). U4 is the riskiest diff and carries a characterization-first execution note.
...
- **Approach:** Module layout mirrors `static/timeline.js` (header comment, Tunables, pure helpers, controller object, Public API, `__internals`). Each push stores an immutable raw copy with an absolute derived time `t = receiveWallClock − ageSeconds` (inputs already stamped as `spot.__recvMs`/`spot.__recvAge` at static/app.js:1973-1974, chosen exactly because they stay stable while the prune mutates `ageSeconds`). Dedup on full spot identity (sourceType, band, sender, receiver, locator, reporterLocator, lat, lng, snr, derived t) — the live wire strips sender/receiver for non-dxcluster spots, so a band/sender/receiver key collapses distinct same-second spots during FT8/WSPR bursts; this mirrors the existing `spotKey` precedent (static/app.js ~1890) and the historySpot field shape U7's parity fixture compares. Receive-time ordering maintained. Two-dimensional eviction on push: spots older than 6h evicted, batch-oldest drop when the count passes the KTD-6 cap. Coverage tracked as contiguous receive intervals so SSE outages create real gaps. Public surface: push, coverage check for an exact window, slice to a t-sorted array, filter re-slice (client-side replica of `streamClientFilter.spotAllowed`: enabled-band set plus SNR thresholds applied only to mqtt and rbn source types, dxcluster/wspr exempt — per KTD-13 selectedBand is not applied), clear.
...
- **Test scenarios:**
  ...
  - Fatal SSE drop mid-timeline: one silent restart attempt; on second failure the stale-coverage state is surfaced (the app's existing stream-status/error surface, copy distinguishing "ring coverage ended" from a connection error).
  - Theme toggle mid-replay re-renders the current moment in the new theme.
  - Surroundings toggle mid-timeline: timeline exits, ring cleared, one fresh stream starts.
