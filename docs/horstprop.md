...
| ⚠️ A2-gap | RBN availability | **RBN raw telnet ingest now exists** (`-rbn-enable`, `telnet.reversebeacon.net:7000` CW/RTTY; no password, but the relay prompts for a callsign — set `-rbn-callsign`/`RBN_CALLSIGN`; `rbn.go`), landing in `dx_raw_spots` with `source_type='rbn'` and a real `mode` (CW/RTTY). **Minimal scope:** RBN feeds the count-based activity chart + (when QRZ resolves a DX locator) the live stream — but is deliberately **kept out of the FT8-SNR conditions baseline/confidence/`classifyMode`** (it never calls `DxBaselineEngine.Observe`, and `isNonConditionsMode` excludes its modes from the FT8 `bandAcc` accumulator) because RBN CW dB (0–40, ~500 Hz filter) is on a different scale than PSKReporter FT8 SNR (−24..+5, 2500 Hz). **Full integration** (calibrated CW/RTTY SNR → comparable conditions score, source-aware `classifyMode`, distance tiers for RBN) is a **future phase** pending the calibration curves in §5. Until then, treat RBN-derived CW/RTTY skimmer SNR as an activity/count input only, not a calibrated link-quality input. |
